...
import it.tim.dashboard.exception.SubsystemException;
...
abstract class ProxyTemplate {
	 ...
	 *            Client's response
	 * @param <T>
	 *            The type of content returned by Client
	 * @return if valid, the body included in the respose
	 * @throws SubsystemException
	 *             if the response is not validated
	 */

	<T> T getBody(ResponseEntity<T> responseEntity) {
		if (validate(responseEntity))
			return responseEntity.getBody();
		else
			throw new SubsystemException(getSubsystemName(), getHeader(responseEntity, ERROR_CLASS_HEADER),
	 ...
	 * @param <T>
	 *            the type of content returned by Client
	 * @return true if response is in OK status, false otherwise
	 */
	<T> boolean validate(ResponseEntity<T> responseEntity) {
	 ...
	 * 
	 * @param throwable
	 *            the error that generated the circuit breaking
	 * @param <T>
	 *            the type of content returned by Client
